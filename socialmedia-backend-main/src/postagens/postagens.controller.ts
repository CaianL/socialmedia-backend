...
@Public()
@Post(":id/comentarios")
async createComment(@Param("id") id: number, @Body() commentDto: CommentDTO) {
  const postagem = await this.postagemRepository.findOne({ where: { id }, relations: ["usuario"] });
  if (!postagem) {
    throw new NotFoundException("Postagem não encontrada");
  }

  const usuario = await this.userRepository.findOneBy({ id: commentDto.usuarioId });
  if (!usuario) {
    throw new NotFoundException("Usuário não encontrado");
  }

  const comentario = this.commentRepository.create({
    ...commentDto,
    postagem,
    usuario,
  });

  return this.commentRepository.save(comentario);
}
